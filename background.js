...
async function activateTab(currentTab, commandRepetition, direction) {
    try {
        ...
        switch (direction) {
            ...
            case "prev":
                if (commandRepetition == "") {
                    tabIndex = positiveModulo((index - 1) , tabCount);
                } else if (commandRepetition == "0") {
                    //Some dev-minded person tried to navigate 0gt :)
                    tabIndex = 0;
                }       
                if(index -commandRepetition -1 < 0)
                    tabIndex = tabCount - positiveModulo(index - commandRepetition-1, tabCount);
                else
                    tabIndex = positiveModulo(index - commandRepetition-1, tabCount);
                break;
